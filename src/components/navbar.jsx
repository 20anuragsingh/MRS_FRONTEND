  ...
  return (
    <div className="browse-page">
      <header className="browse-nav">
        ...
        <div className="nav-actions"><button className="search-toggle" onClick={() => setSearchOpen(!searchOpen)} aria-label="Search">⌕</button><button className="kids" onClick={() => goTo('shows')}>TV</button><button className="bell" onClick={() => setNotificationsOpen(!notificationsOpen)} aria-label="Notifications">♢</button><button className="avatar" onClick={logout} aria-label="Log out" title="Log out">A</button></div>
      </header>
      {searchOpen && <Search />}
